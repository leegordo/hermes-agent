...
elif [ -f "/root/.env" ] && grep -q "^GITHUB_TOKEN=" "/root/.env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "/root/.env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    if [ -n "$GITHUB_TOKEN" ] && [ "$GITHUB_TOKEN" != "***" ]; then
        GH_AUTH_METHOD="curl"
    fi
elif [ -f "/root/.env" ] && grep -q "^GITHUB_CLASSIC_TOKEN=" "/root/.env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_CLASSIC_TOKEN=" "/root/.env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    if [ -n "$GITHUB_TOKEN" ] && [ "$GITHUB_TOKEN" != "***" ]; then
        GH_AUTH_METHOD="curl"
    fi
...
# Fallback: gh may have a valid token even if auth status check failed
if [ "$GH_AUTH_METHOD" = "none" ] && command -v gh &>/dev/null; then
