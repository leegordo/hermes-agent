...
- onboarding flows
- dashboard concepts
- **portfolio review and redesign** — see Portfolio Redesign section below
- **AI prompt-to-artifact tools** — see AI Prompt Tool Pattern section below

## AI Prompt Tool Pattern

When building AI-powered design generation tools (prototype factories, UI generators, concept galleries), use the **3-zone layout pattern** documented in `references/ai-prompt-tool-pattern.md`.

This pattern covers:
- Left sidebar: project navigation + agent-generated example gallery
- Center canvas: live preview viewport with resize handles
- Bottom floating prompt bar: prompt input, type/model selectors, Create button
- Working modals: Type selector, Model selector, Figma import, Pricing/upgrade

**Key user preference:** The user explicitly rejects community/social features (likes, followers, public feeds). The gallery should be framed as a personal workspace of agent-generated examples, not a social platform. Use language like "Your projects" and "Agent gallery" rather than "Community" and "Feed."

## Portfolio Redesign — Specific Guidance

When a user asks you to review or redesign their portfolio, treat it as a **strategic positioning exercise**, not just a visual refresh.

### Audit Before Designing
Always inspect both the **live site** (screenshots, accessibility tree) AND the **repo code** (package.json, page structure, components, content files) before critiquing. The repo reveals the actual architecture; the live site reveals the user experience.

### The Portfolio Assessment Framework
Score across these dimensions:
1. **Positioning** — Does the hero immediately answer: what do you do, for whom, and why should I care? Generic titles like "Product Designer" fail. Specific transformations like "I help AI startups turn prototypes into products people use" win.
2. **Proof of Value** — Are there metrics, testimonials, client logos, or process evidence? A portfolio without proof is just a resume with better typography.
3. **Visual Distinctiveness** — Does it look like a template (dark mode, gradient overlays, card grid, Geist font, tracking-wide labels)? If yes, the site is invisible.
4. **Case Study Quality** — Do projects show problem → role → process → outcome? Or just name, year, and role tags?
5. **Hero Effectiveness** — Is the hero section selling transformation or just stating identity? The highest-value real estate on the site.
6. **Technical Execution** — Broken images, missing Open Graph, performance issues. Credibility killers.

### Brand Asset Integration
Before generating any visual mockups or design directions, **fetch the user's actual brand assets** — logo, favicon, color palette, typography — from their live site or repo. Never use placeholder text or generic logos in portfolio explorations.

1. **Logo** — Check `/images/logo.*`, `/logo.*`, or inspect the live site's `<img>` tags. Download and embed as base64 for self-contained mockups.
2. **Colors** — Extract from CSS variables, Tailwind config, or screenshot analysis. Don't invent a new palette unless the user asks.
3. **Fonts** — Use the actual font families from the site. If they're Google Fonts, import them. If custom, fall back to the closest system match.

**Pitfall:** The user may have spent significant time on their brand identity. Using a placeholder logo signals that you didn't look at their actual work. When in doubt, ask: "Should I use your existing logo and colors, or explore new directions?"

### Redesign Strategy: Two Paths
When pitching fixes, always offer two paths:

- **Path A (Surgical)** — Keep the stack, restructure content and visual hierarchy. Rewrite copy, add social proof, restructure case studies, adjust visual language. 1–2 days. See `references/portfolio-surgical-rewrite.md` for the exact execution checklist.
- **Path B (Full Redesign)** — New visual direction, custom typography, scroll-driven case studies, interactive motion. 1 week. Do Path A first, then Path B once Path A is converting.

### Iterative Design Workflow
When the user asks to "see directions" or "explore variants":

1. **First pass** — 2–3 visually distinct directions using their real brand assets. Host for review.
2. **User picks** — They'll usually favor one or two directions.
3. **Second pass** — Push deeper into the favored direction(s). Iterate on typography, spacing, layout, and content. Keep hosting updated previews.
4. **Consolidate** — Once a direction is locked, port it into the actual codebase (Next.js, etc.). Don't leave the project as a pile of mockups.

**Pitfall:** Don't generate 3+ directions, get feedback, then generate 3 more new directions. Iterate *within* the chosen direction instead of restarting from scratch each round.

### Anti-Patterns Specific to Portfolios
...
- Dark mode + gradient overlays + tracking-wide uppercase as a substitute for a point of view
- Presenting projects as "name + year + role" instead of "problem + transformation + outcome"
- No mention of niche or differentiator
